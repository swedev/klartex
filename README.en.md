...
> **Always reference files by plain name** — `\includegraphics{logo.pdf}`, not `\includegraphics{./logo.pdf}`. TeX's file lookup (Kpathsea) does not search for names starting with `./` or `../`; it tries them as-is against xelatex's working directory, which is a temporary build directory. Such references therefore never resolve, neither in the template's directory nor in the working directory.
