...
Custom slot sources come from `--header-template` / `--footer-template` on the CLI (one slot each; both files must share a directory, which becomes `asset_dir`) or `header_source` / `footer_source` on `render()`. There is no whole-page source: a custom slot owns its own chrome, the other slot and the document-level settings (`font`, `header_font`, `diff_style`) still come from `data.page_template`. Slot content lives under `fields` on both slots (`footer: {"variant": "columns", "fields": {"company": …}}`); a variant's own settings, like `pagenumber`'s `title`, sit beside `variant`.
