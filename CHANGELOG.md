...
- **Receptschemana är stängda.** Okända nycklar på alla objektnivåer i `protokoll`, `faktura`, `kvitto`, `resultatrakning`, `balansrakning`, `budgetrapport` och `sie-exportrapport` avvisas med ett valideringsfel som namnger nyckeln (`detail.path` pekar på det omslutande objektet), i stället för att tyst ignoreras — samma löfte som blockmotorns scheman redan ger. Ett guard-test går igenom varje laddat receptschema så ett nytt recept inte kan öppna nivåer igen. (#78)
- **Faktura och kvitto: klassoptionen `narrowmargins` och `document.class_options` är borttagna.** Recepten deklarerar sina marginaler (2 cm sidor, 1,7 cm topp) som recept-default under `page_template.margins`, som payloaden overridar per nyckel; PDF:erna är oförändrade. En payload som lägger till ett *renderande* fördefinierat sidhuvud (letterhead med `org_name`, eller `logo` med fil) på faktura/kvitto måste också skicka `margins.top` (t.ex. `3.4cm`). En egen sidhuvudskälla påverkas inte — receptets default-`top` tillämpas inte när en källa äger sidhuvudsslotten. (#96)

### New features
- **`klartex.validate(template_name, data)`.** Exakt den validering `render()` kör — registeruppslag, JSON-schema och per-block-kontrollen för blockmotorn — utan att röra TeX-verktygskedjan; `render()` anropar den först så de två inte kan glida isär. Samma undantag (`jsonschema.ValidationError`, `BlockValidationError` med `path`). Sidmallskomposition och fontpreflight ligger kvar vid rendering. CLI: `klartex validate -d data.json [-t mall]`. (#109)
- **`BlockValidationError` med `path`.** Blockvalideringsfel från `render()` bär noden som föll (`["body", 3]`, `["body", 2, "content", 0]`, för schemafel blockpositionen plus `absolute_path`) som attribut, med oförändrad meddelandetext; `ValueError`-subklass så befintliga `except` fortsätter fungera. `klartex serve` läser attributet i stället för att tolka meddelandet. (#76)
...
### Fixes
- **Kolumnfotens `email`/`web` bryts inuti kolumnen.** Samma brytmöjligheter efter `@`, `.` och `/` som letterhead-sidhuvudet fick i 0.19.0, plus radbrytande celler för kontaktraderna — en lång adress i Företag-kolumnen rinner inte längre över kolumnkanten. (#104)
