...
**Recipe path** (specialized surface for stable transactional document types) — `klartex/templates/<name>/recipe.yaml` declares which components and metadata fields make up a fixed document type. `recipe.py::prepare_recipe_context` resolves dot-paths (e.g. `data_map: {items: agenda_items}`) and hands a context to `_recipe_base.tex.jinja`. Used by `protokoll`, `faktura`, `balansrakning`, `resultatrakning`, `budgetrapport`, `sie-exportrapport`. The value is letting producers send domain-shaped JSON (e.g. an invoice with `lines[]`) without describing layout — useful for upstream systems with stable contracts. A recipe declares its own default slots under `document.page_template`; a footer slot may add a `fields_from` map (footer field → dot-path, or list of dot-paths) that `load_recipe` validates and `prepare_recipe_context` resolves per render and merges **under** the payload's own footer of that variant, key by key. `document.label_missing_footer_fields` turns on the columns footer's `\kxfooterlabelmissingtrue` mode, where an empty column names its gap in `kxmuted` grey instead of being dropped. Both are used by `faktura` and `kvitto`, whose footer is derived from `sender` and the payment fields.
...
`detail.path` comes from the core in both shapes `render.py` maps: `BlockValidationError.path` for block failures, `ValidationError.absolute_path` for schema violations.
